...
class Net_conv3dbis(nn.Module):
	...
	def forward(self, x):
		...
		x2 = torch.zeros(x.size(0),2,self.nb_output)
		for j in range(x.size(2)):
			x2[:,j,:] = F.relu(self.fc2(x1[:,j,:].view(-1,self.nb_hidden)))
		#print('[step4] : {0}'.format(x2.size()))
		return x2
	...
	def freeze_features(self, q):
		...
		# q = True means that it is frozen and we do NOT need the gradient
			p.requires_grad = not q
######################################################################
class Net_targetTheo(nn.Module):
	def __init__(self,m):
		super(Net_targetTheo,self).__init__()
		self.model_f = m.forward
	def forward(self,x):
		x = self.model_f(x)
		print(x.mean())
		return class_to_target(x,True)
